...
export const StoryChallengeView: React.FC<StoryChallengeViewProps> = ({ projectId: _projectId }) => {
  ...
  return (
    <div className="flex-1 overflow-hidden bg-[#f6f8fb] dark:bg-[#0b0d10] text-zinc-900 dark:text-zinc-100 flex flex-col font-sans relative transition-colors">
      {/* Workspace Header */}
      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800/80 flex items-center justify-between shrink-0 bg-white dark:bg-[#151922] shadow-2xs">
        <div>
        ...
        <div className="flex items-center gap-3 font-mono text-xs">
          <span className="px-3 py-1 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl text-zinc-700 dark:text-zinc-300 flex items-center gap-1.5 font-semibold">
            <Clock className="w-3.5 h-3.5 text-zinc-400" />
            Budget: {budgetRemaining}/10
          </span>
          <span className="px-3 py-1 bg-blue-50 dark:bg-zinc-900 border border-blue-200 dark:border-zinc-800 rounded-xl text-blue-700 dark:text-zinc-300 flex items-center gap-1.5 font-semibold">
      ...
      {sessionCompleted ? (
        ...
        <div className="flex-1 overflow-y-auto p-8 flex items-center justify-center">
          <div className="max-w-md w-full border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-8 bg-white dark:bg-[#151922] text-center space-y-6 shadow-sm">
            <Trophy className="w-16 h-16 text-amber-500 mx-auto" />
            <div>
              <h2 className="text-2xl font-extrabold text-zinc-950 dark:text-white">Scenario Complete!</h2>
            ...
            <button
              ...
              className="w-full py-3 bg-blue-600 dark:bg-white text-white dark:text-zinc-950 font-bold rounded-xl hover:bg-blue-500 dark:hover:bg-zinc-200 transition text-xs flex items-center justify-center gap-2 cursor-pointer shadow-xs"
            >
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-56 border-r border-zinc-200 dark:border-zinc-800/80 p-4 bg-white dark:bg-[#101318] overflow-y-auto shrink-0 font-mono text-xs space-y-2">
              ...
              return (
                <div
                  ...
                  className={`p-2.5 rounded-xl border flex items-center gap-2.5 transition ${
                    isCurrent
                      ? 'border-blue-500 bg-blue-50/50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 font-bold shadow-xs'
                      : isDone
                      ? 'border-zinc-200 dark:border-zinc-850 bg-zinc-50 dark:bg-[#151922] text-zinc-500 dark:text-zinc-400'
                ...
                >
                  <span className="shrink-0">
                    {isDone ? (
                      ...
                      <span className="w-2.5 h-2.5 rounded-full border border-zinc-300 dark:border-zinc-700 block" />
                    )}
                  </span>
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#f6f8fb] dark:bg-[#0b0d10] space-y-6">
            <div className="border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-6 bg-white dark:bg-[#151922] shadow-xs space-y-6">
              <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-3 font-mono text-xs">
                <span className="text-zinc-500 dark:text-zinc-400 uppercase font-bold">
                  Decision Point {currentPoint.id} of {OS_STORY_STEPS.length}
                </span>
                <span className="text-blue-600 dark:text-blue-400 font-semibold">Operating Systems</span>
              </div>

              <div>
                <h2 className="text-lg font-bold text-zinc-950 dark:text-white">{currentPoint.title}</h2>
                <p className="text-xs text-zinc-700 dark:text-zinc-300 mt-3 leading-relaxed font-mono bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-200 dark:border-zinc-850">
              ...
              <div className="space-y-3">
                {currentPoint.options.map((opt) => {
                  ...
                  let optStyle = 'border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-[#101318] hover:border-zinc-300 dark:hover:border-zinc-700 text-zinc-800 dark:text-zinc-200';
                  if (isAnswered) {
                    if (opt.is_correct) optStyle = 'border-emerald-500/80 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-200 font-semibold';
                    else if (isSelected && !opt.is_correct) optStyle = 'border-rose-500/80 bg-rose-50 dark:bg-rose-950/20 text-rose-800 dark:text-rose-200';
                  } else if (isSelected) {
                    optStyle = 'border-blue-500 bg-blue-50/50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 font-semibold shadow-xs';
                  }
                  ...
                  return (
                    <button
                      ...
                      disabled={isAnswered}
                      className={`w-full p-4 rounded-xl border text-left text-xs transition flex items-center justify-between cursor-pointer ${optStyle}`}
                    >
                      <span>{opt.text}</span>
                      {isAnswered && opt.is_correct && <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />}
                      {isAnswered && isSelected && !opt.is_correct && <XCircle className="w-4 h-4 text-rose-500 shrink-0" />}
                    </button>
                  );
                })}
              </div>
              ...
              <div className="pt-2 flex justify-end">
                {!isAnswered ? (
                  <button
                    ...
                    className="px-6 py-2.5 bg-blue-600 dark:bg-white text-white dark:text-zinc-950 font-bold rounded-xl hover:bg-blue-500 dark:hover:bg-zinc-200 transition text-xs disabled:opacity-50 cursor-pointer shadow-xs"
                  >
                    Submit Choice
                  </button>
                ) : (
                  <button
                    ...
                    className="px-6 py-2.5 bg-blue-600 dark:bg-white text-white dark:text-zinc-950 font-bold rounded-xl hover:bg-blue-500 dark:hover:bg-zinc-200 transition text-xs flex items-center gap-2 cursor-pointer shadow-xs"
                  >
                    Next Decision Point
          ...
          <div className="w-72 border-l border-zinc-200 dark:border-zinc-800/80 p-5 bg-white dark:bg-[#101318] overflow-y-auto shrink-0 space-y-6 font-mono text-xs">
            ...
            {/* Current State */}
            <div className="border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-4 bg-zinc-50/50 dark:bg-[#151922] space-y-3 shadow-2xs">
              <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">CURRENT STATE</span>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-zinc-500 dark:text-zinc-400">Mastery</span>
                  <span className="text-zinc-950 dark:text-white font-bold font-mono">{(currentMastery * 100).toFixed(0)}%</span>
                </div>
                <div className="w-full h-1.5 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 transition-all duration-300" style={{ width: `${currentMastery * 100}%` }} />
                </div>

                <div className="flex justify-between items-center pt-1">
                  <span className="text-zinc-500 dark:text-zinc-400">Confidence</span>
                  <span className="text-zinc-800 dark:text-zinc-200">{(confidence * 100).toFixed(0)}%</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-zinc-500 dark:text-zinc-400">Difficulty Fit</span>
                  <span className="text-zinc-800 dark:text-zinc-200">{learnerAbility > 0.6 ? 'Hard' : learnerAbility > 0.4 ? 'Medium' : 'Easy'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-zinc-500 dark:text-zinc-400">Misconceptions</span>
                  <span className={hasMisconception ? 'text-amber-500 font-bold' : 'text-emerald-500'}>
                    {hasMisconception ? '1 Detected' : 'None'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-zinc-500 dark:text-zinc-400">Remaining Budget</span>
                  <span className="text-zinc-800 dark:text-zinc-200">{budgetRemaining} / 10</span>
                </div>
              </div>
            </div>
            ...
            <div className="border border-zinc-200 dark:border-zinc-800/80 rounded-2xl p-4 bg-zinc-50/50 dark:bg-[#151922] space-y-3 shadow-2xs">
              ...
              <div>
                <h4 className="text-xs font-bold text-zinc-950 dark:text-white uppercase">{selectedIntervention}</h4>
